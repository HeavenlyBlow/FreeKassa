...
namespace FreeKassa.KKT
{
    public class KKTManager : IDisposable
    {
        ...
        public KKTManager(bool manualShiftManagement, PrinterManager printerManager, Model.KKT kktSettings)
        {
        ...
        public KKTManager(Model.KKT kktSettings)
        {
            if (!ValidationKktSettings.Check(kktSettings))
                throw new SettingsExceptions("Отсутсвует имя кассира или включены обра режима смеен");
            _kktModel = kktSettings;
        ...
        private CloseShiftsFormModel DataAboutCloseShift()
       {
        ...
        {
            _kktModel = ConfigHelper.GetSettings().KKT;
        }
